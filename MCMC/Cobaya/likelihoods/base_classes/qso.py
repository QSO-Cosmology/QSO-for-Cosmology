...
:Synopsis: QSO likelihoods.
:Author: Micol Benetti and Giada Bargiacchi 

This code provides qso likelihood
It was created by adapting the sn file for QSO data.
No lines unrelated to the QSO analysis were removed; the file was only minimally modified to allow reading and using the QSO data.
-----

To use any of these likelihoods, simply mention them in the likelihoods block
(e.g. ``qso.qso``), or add them
using the :doc:`input generator <cosmo_basic_runs>`.

Installation
------------

This likelihood can't be installed automatically `.
"""
# Global
import numpy as np
import os
...
class QSO(DataSetLikelihood):
    # Data type for aggregated chi2 (case sensitive)
    type = "QSO"

    install_options = {"github_repository": "CobayaSampler/qso_data",
                       "github_release": "v1.6"}
    ...
    def init_params(self, ini):
        ...
        self.nsn = ix
        self.log.debug('Number of QSO read: %s ' % self.nsn)
    ...
    def alpha_beta_logp(self, lumdists, alpha=0, beta=0, k=0, invcovmat=None):
        if self.alphabeta_covmat:
            if self.use_abs_mag:
                self.log.warning("You seem to be using JLA with the absolute magnitude "
                                 "module. JLA uses a different calibration, the k "
                                 "module only works with QSO!")
                estimated_scriptm = k + 25
            else:
                alphasq = alpha * alpha
                betasq = beta * beta
                alphabeta = alpha * beta
        ...
        if self.marginalize:
            ...
            if self.alphabeta_covmat:
                return self.alpha_beta_logp(lumdists, params_values[self.alpha_name],
                                            params_values[self.beta_name], k)
            else:
                return self.alpha_beta_logp(lumdists, k=k)
